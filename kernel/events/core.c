...
static int perf_event_open_from_kernel(struct perf_event_attr *attr,
			pid_t pid, int cpu, int group_fd, unsigned long flags)
{
	struct perf_event *group_leader = NULL, *output_event = NULL;
	struct perf_event_pmu_context *pmu_ctx;
			...
			uint32_t nr_pages, uint64_t freq)
{
	...
	event_fd = perf_event_open_from_kernel(&attr, -1, cpu, -1, 0);
	if (event_fd < 0) {
		printk(KERN_ERR
...
struct perf_event_read_samples {
	struct perf_buffer *rb;
	uint64_t data_head, data_tail;
	uint32_t sample_sz;
};
struct perf_event_read_samples *__reader;

int perf_setup_event_for_reading(struct perf_event *event)
{
	int ret = 0;

	__reader = kzalloc(sizeof(struct perf_event_read_samples), GFP_KERNEL);
	if (!__reader) {
		return -ENOMEM;
	}

	__reader->rb = event->rb;
	if (!__reader->rb) {
		ret = -ENOMEM;
		goto free_out;
	}

	__reader->data_head = READ_ONCE(__reader->rb->user_page->data_head);
	__reader->data_tail = READ_ONCE(__reader->rb->user_page->data_tail);
	smp_rmb();

	if (__reader->data_head == __reader->data_tail) {
		ret = -ENOENT;
		goto free_out;
	}
...
void* perf_get_event_sample(int *ret)
{
	...
	int pgshift = PAGE_SHIFT + page_order(__reader->rb);
	uint64_t pgindex = (__reader->data_tail >> pgshift) &
				(__reader->rb->nr_pages - 1);
	uint64_t offset = __reader->data_tail & ((1 << pgshift) - 1);

	header = (struct perf_event_header *)
			(__reader->rb->data_pages[pgindex] + offset);
	...
	__reader->sample_sz = header->size;
	*ret = 0;
	return (void *)header;
}
...
{
	smp_rmb();
	__reader->data_tail += __reader->sample_sz;
	WRITE_ONCE(__reader->rb->user_page->data_tail, __reader->data_tail);
}
